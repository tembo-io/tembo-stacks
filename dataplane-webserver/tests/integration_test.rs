...
mod tests {
    ...
    fn format_prometheus_query(url: &str, query: &str, start: u64) -> String {
        ...
        let url = url.as_str();
        let query_url = Url::parse_with_params(url, &query_params).expect("Failed to format query parameters").to_string();
    ...
    async fn test_metrics_query_range() {
        ...
        // 24 hours ago, unix time
        let start = (SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).expect("Failed to get UNIX time").as_secs() - 24 * 60 * 60);
        let query_url = format_prometheus_query(url, query, start);
        let req = test::TestRequest::get().uri(query_url.as_str()).to_request();
        let resp = test::call_service(&app, req).await;
        assert!(resp.status().is_success());

        // 25 hours ago, unix time
        let start = (SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).expect("Failed to get UNIX time").as_secs() - 25 * 60 * 60);
        let query_url = format_prometheus_query(url, query, start);
        let req = test::TestRequest::get().uri(query_url.as_str()).to_request();
        let resp = test::call_service(&app, req).await;
        ...
        let start = (SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).expect("Failed to get UNIX time").as_secs() - 1 * 60 * 60);
