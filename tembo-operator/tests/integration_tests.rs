...
mod test {
    ...
    // #[tokio::test]
    // #[ignore]
    // async fn functional_test_restore_ha_shared_preload_libraries() {
    //     // Initialize the Kubernetes client
    //     let client = kube_client().await;
    //     let state = State::default();
    //     let context = state.create_context(client.clone());
    ...
    //     let lp = ListParams::default()
    //         .labels(format!("app=postgres-exporter,coredb.io/name={}", restore_name).as_str());
    //     let restore_exporter_pods = restore_pods.list(&lp).await.expect("could not get pods");
    //     let restore_exporter_pod_name = restore_exporter_pods.items[0].metadata.name.as_ref().unwrap();
    //     pod_ready_and_running(restore_pods.clone(), restore_exporter_pod_name.clone()).await;
    //
    //     // Assert that we can query the database with \dx;
    //     let result = psql_with_retry(context.clone(), coredb_resource.clone(), "\\dx".to_string()).await;
    //     assert!(result.stdout.clone().unwrap().contains("plpgsql"));
