...
impl CoreDB {
    ...
    async fn reconcile(&self, ctx: Arc<Context>, cfg: &Config) -> Result<Action, Action> {
        ...
        new_status.last_fully_reconciled_at = {
            let current_fully_reconciled_at = self.status.clone().unwrap_or_default().last_fully_reconciled_at;
            // Update the timestamp if it's been more than 10 seconds since the last update
            if current_fully_reconciled_at.map_or(true, |last_reconciled| current_time > last_reconciled + Duration::from_secs(10)) {
                Some(current_time)
            } else {
